...
import Membership, { OrganizationRole, SystemRole } from '@requisite/model/lib/user/Membership';
...
jest.mock('sequelize', () => {
    ...
    const mockOrgMemberships: Membership<Organization>[] = [{
        id: 0,
        userId: mockUsers.org0Owner.id,
        user: mockUsers.org0Owner, // org0Owner
        entity: mockOrgs[0],
        role: OrganizationRole.OWNER
    } as unknown as Membership<Organization>, {
        id: 1,
        userId: mockUsers.org0Member.id,
        user: mockUsers.org0Member, // org0Member
        entity: mockOrgs[0],
        role: OrganizationRole.MEMBER
    } as unknown as Membership<Organization>, {
        id: 2,
        userId: mockUsers.org0and1Member.id,
        user: mockUsers.org0and1Member, // org0and1Member
        entity: mockOrgs[0],
        role: OrganizationRole.MEMBER
    } as unknown as Membership<Organization>, {
        id: 3,
                    ...
                    switch(this.name) {
                        ...
                        case 'UsersDataModel':
                            mockModels[this.name] = new MockModel<User>(
                                // initial user data
                                Object.keys(mockUsers).map(key => mockUsers[key]),
